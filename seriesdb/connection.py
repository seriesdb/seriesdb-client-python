...
import seriesdb.protocol.seriesdb_protocol_ext as protocol_ext
from seriesdb.listenable import Listenable
...
class Connection(Listenable):
    ...
    async def request(self, msg, timeout=10):
        round_ref = self.__new_round_ref()
        msg.round_ref = round_ref

        self.__arrived_events[round_ref] = asyncio.Event(loop=self.__loop)
        await self.__send(msg)

        event = self.__arrived_events.get(round_ref)
        try:
            await asyncio.wait_for(event.wait(), timeout)
        except Exception as e:
            raise e
        finally:
            self.__arrived_events.pop(round_ref)

        code, msg = self.__msgs.pop(round_ref)
        if code != Code.OK:
    ...
    async def __ping(self):
        if self.__websocket != None:
            await self.__send(protocol_types.PingReq())
    ...
    async def __send(self, msg):
        encoded_msg = None
        try:
            encoded_msg = protocol_ext.encode_msg(msg)
        except Exception:
    ...
    def __on_msg(self, msg):
        ...
        if event == None:
            ...
            return
        if msg.__class__ == protocol_types.ErrorRep:
            self.__msgs[round_ref] = (msg.code, msg.desc)
        else:
            self.__msgs[round_ref] = (Code.OK, msg)
        event.set()

    # ===========================================
    # utils
    # ===========================================
    def __new_round_ref(self):
        new_round_ref = self.__last_round_ref + 1
        if new_round_ref > 600000:
            new_round_ref = 1
        self.__last_round_ref = new_round_ref
